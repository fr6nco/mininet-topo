...
class CustomLinearTopo(Topo):
    ...
    def __init__(self, switchcount, bwlimit):
    ...
    def build(self):
        ...
        for switch in switches:
            if last:
                self.addLink( last, switch, cls=TCLink, bw=self.bwlimit)
            last = switch

def setup(count, bwlimit):
    "Start Network"
    topo = CustomLinearTopo(count, bwlimit)
...
if __name__ == '__main__':
    ...
    try:
        print "Setting link limits between switches to " + str(int(sys.argv[2])) + " Mbit/s"
    except ValueError:
        print "argument must be integer"
